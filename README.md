...
- 每个 worker 按固定时间片执行“忙等 + 休眠”，从而控制目标 CPU 占用率
- 支持按“整机总 CPU 占用率”模式运行，程序会自动识别逻辑核数并拆分 worker
- 支持按“可分配内存百分比”预留内存，并逐页写入确保实际占用
- 会把整机总 CPU / 内存使用率按秒写入可执行文件同级目录的 `cpu_memory_stress.log`
...
- 日志文件路径默认是可执行文件同级目录下的 `cpu_memory_stress.log`
